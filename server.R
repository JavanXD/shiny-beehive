...
server <- function(input, output, session) {
  ...
  ########################################
  format <- "%Y/%m/%d %H:%M:%S"
  # import csv and set timeformat
  beehive_df <- reactiveValues()
  beehive_df <- read_csv("beehive.csv", col_types = cols(
    unixtime = col_number(),
  ...
  output$spearmanUI <- renderUI({
    tags$div(
      ...
      br(),
      h3("Spearman Korrelationskoeffizient"),
  ...
  output$corUI <- renderUI({
    ...
    tags$div(
      ...
      h4("Zusammenfassung der Gruppen"),
      verbatimTextOutput("kruskalDf"),
      h4("Visualisierung mit Hilfe von Boxplots und Meanplots"),
      plotOutput("kruskalBoxplot"),
      plotOutput("kruskalMean"),
      h4("Berechnung mit Kruskal-Wallis Tests"),
      ...
      h4("Interpretation und Anwendung eines Mehrfachpaarvergleiches"),
  ...
  change_dataframe <- reactive({
    ...
    if (!is.null(input$fileuploadFile)) {
      ...
      beehive_df_unfiltered <<- read_csv(input$fileuploadFile$datapath, col_types = cols(
        unixtime = col_number(),
        timestamp = col_datetime(format = format)
      ))
  ...
  ########################################
  selectDay <- function(id = "selectedDay", val = "2019-06-7", text = "Datum auswählen") {
    dateInput(id, text,
      value = val,
  ...
  selectDaysCount <- function(id = "selectedDaysCount", val = 30, text = "Anzahl Tage auswählen") {
    sliderInput(id, text, 1, 180, val,
      step = 1, round = FALSE,
  ...
  output$gewichtsVerlaufDailyPlot <- renderPlotly({
    ...
    plot_ly(final_df, x = ~hour, y = ~weight1, name = "vor 1 Tag", type = "scatter", mode = "lines+markers") %>%
      ...
      layout(
        xaxis = list(title = "Uhrzeit", tickmode = "linear", tickformat = "Uhr", range = c(0, 23)),
  ...
  output$downloadData <- downloadHandler(
    ...
    content = function(con) {
      beehive_df <- transform(beehive_df, timestamp = format(timestamp, format)) # fix dateformat (turn 2018-04-05 08:15:45 into 2018/04/05 08:15:45)
      write.table(beehive_df, con, sep = ",", dec = ".", na = "", quote = FALSE, row.names = F, col.names = T)
    },
