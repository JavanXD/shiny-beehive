...
dashboard <- tabBox(
  ...
  tabPanel("Korrelation", icon = icon("project-diagram"), uiOutput("spearmanUI")),
  tabPanel("Kruskal-Wallis", icon = icon("thermometer"), uiOutput("corUI")),
...
info <- tags$div(
  ...
  # Div mit Copyrights
  tags$div(
    style = "padding:10px;color:#a1afb6",
...
ui <- tagList(
  ...
  dashboardPage(
    skin = "yellow", title = "HoneyPi Shiny App",
    header,
    sidebar,
    body
  ),
